      ...
      <Container>
        ...
        <div className="grid md:grid-cols-3 gap-px bg-line">
          {steps.map((s, i) => (
            <div
              key={s.n}
              className={`reveal reveal-d${i + 1} bg-bg p-8 lg:p-10`}
            >
              <div className="mono text-xs text-muted tracking-[0.18em] mb-6">
                {s.n}
              </div>
              <h3 className="text-xl font-medium mb-3 text-balance">{s.title}</h3>
...
export function Coverage() {
  useReveal();
  return (
    <section id="coverage" className="py-32 border-t border-line">
      <Container>
        <div className="reveal max-w-2xl mb-16">
        ...
        <div className="grid sm:grid-cols-2 gap-px bg-line border border-line">
          {CHECK_ITEMS.map((c, i) => (
            <div
              key={c.name}
              className={`reveal reveal-d${(i % 4) + 1} bg-bg p-5 flex items-start gap-4`}
            >
              <c.Icon className="w-5 h-5 shrink-0 text-green mt-0.5" aria-hidden />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium">{c.name}</div>
                <div className="mono text-[10.5px] uppercase tracking-[0.16em] text-muted mt-1">
                  {c.cat}
                </div>
              </div>
            </div>
      ...
      <Container>
        ...
        <div className="divide-y divide-line border-y border-line max-w-3xl">
          {FAQ_ITEMS.map((item, i) => (
            <details
              key={i}
              className={`reveal reveal-d${(i % 4) + 1} group py-6`}
            >
              <summary className="cursor-pointer list-none flex items-start gap-4 hover:text-green transition-colors">
              ...
              <div className="mt-4 pl-10 text-ink-2 leading-relaxed">
                {item.a}
              </div>
            </details>
          ))}
...
export function CTA() {
  useReveal();
  return (
    <section className="py-32 border-t border-line relative overflow-hidden">
      <div
      ...
      <Container className="relative">
        <div className="reveal max-w-3xl mx-auto text-center">
          <h2 className="text-h2 font-medium mb-5 text-balance">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <a
              href="#audit"
              className="inline-flex items-center gap-2 px-7 py-4 rounded-md bg-green text-bg font-medium hover:bg-green/90 transition-colors"
            >
              Run an audit now
            </a>
            <a
              ...
              className="inline-flex items-center gap-2 px-7 py-4 rounded-md border border-line-strong hover:bg-card transition-colors text-ink-2"
            >
              Hire me
            </a>
          </div>
